...
import imgFront from '@/../public/images/icons/passport.png'
...
export const Passport: React.FC<ComponentProps> = () => {
  ...
  const [passport] = idOptions?.filter((item) => item.text === 'Pasaporte')

  const tabs = [
    { id: 0, label: 'Pasaporte', img: imgFront }
  ]

  const getData = () => {
    const docData = data.validation_document.filter((item: any) => item.document.text === 'Pasaporte')
    if (docData.length === 1) {
  ...
  useEffect(() => {
    if (imageFile !== null && imageFile !== undefined) {
      saveDocument(passport.id, imageFile, '0')
      setFileSent(true)
    }
  }, [imageFile])
