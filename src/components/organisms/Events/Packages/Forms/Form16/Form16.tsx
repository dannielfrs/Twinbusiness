...
import styles from './Form16.module.scss'
...
import { Step16Context } from '@/context/authenticated/events/register/createEvent/step16/Step16Context'
...
const Form16: React.FC<ComponentProps> = ({ goBack, goBackStep }) => {

  const { loading, data, saveData, savingData, savedSuccess } = useContext(Step16Context)
  ...
  return (
    <FormHookProvider methods={methods} onSubmit={onSubmit}>
      <div className={styles.Form16}>
        <Card className={styles.CardSend}>
          {data?.terms && data.terms.length > 0 &&
            <>
              <p className={styles.title}>{data?.terms[0]?.title}</p>
              <div className={styles.contText}>
                <p style={{ whiteSpace: 'pre-line' }} dangerouslySetInnerHTML={{ __html: data?.terms[0]?.description }}></p>
              </div>
            </>
          }
          <Card className={styles.acept}>
            <p>Al aceptar este campo se enviará tu formulario a revisión</p>
            <Checkbox
              name='review'
              inputId='review'
              label=''
          ...
          <div className={styles.contCards}>
            <div className={styles.card1}>
              {data?.terms && data.terms.length > 0 &&
                <Card className={styles.contract}>
                  <p className={styles.title2}>{data?.terms[1]?.title}</p>
                  <p className={styles.text2} dangerouslySetInnerHTML={{ __html: data?.terms[1]?.description }}></p>
                  <div className={styles.terms}>
                    <Link href='#' onClick={() => { setModalIsOpen1(true) }}>Terminos y Condiciones del Contrato</Link>
                    <p className={styles.textBold}>Aceptar</p>
                    <Checkbox
                      name='ticketing_system'
                      inputId='ticketing_system'
                      label=''
                      variant='primary'
                      className={styles.checkbox}
                      rules={{ required: true }}
              ...
              <p className={styles.textBottom}>Al aceptar se activará el sistema de boletaje</p>
            </div>
            <div className={styles.card1}>
              {data?.terms && data.terms.length > 0 &&
                <Card className={styles.contract}>
                  <p className={styles.title2}>{data?.terms[2]?.title}</p>
                  <p className={styles.text2} dangerouslySetInnerHTML={{ __html: data?.terms[2]?.description }}></p>
                  <div className={styles.terms}>
                    <Link href='#' onClick={() => { setModalIsOpen1(true) }}>Terminos y Condiciones del Contrato</Link>
                    <p className={styles.textBold}>Aceptar</p>
                    <Checkbox
                      name='recharge_system'
                      inputId='recharge_system'
                      label=''
                      variant='primary'
                      className={styles.checkbox}
                      rules={{ required: true }}
              ...
              <p className={styles.textBottom}>Al aceptar se activará el sistema de recargas</p>
            </div>
          </div>
        </Card>
          ...
          <div className={styles.TermsAndConditions}>
            <Card className={styles.cardTerms}>
              <div className={styles.ButtonClose}>
                <Button variant='terms' className={styles.close} onClick={() => setModalIsOpen1(false)}>x</Button>
              </div>
              ...
              <div className={styles.contText}>
                ...
                <p className={styles.text}>El acceso a nuestra <b>plataforma web y/o móvil,</b> así como la utilización directa y/o indirecta bajo cualesquiera situaciones
                  ...
                  en las condiciones Generales y sus Anexos, y demás políticas y principios incorporados a las mismas por referencia.</p>
                <br />
                ...
                <p className={styles.text}>El Usuario se compromete a notificar a <b>TWIN</b> en forma inmediata y por medio idóneo y fehaciente, a cualquier uso no autorizado de su
                  ...
                  indemnización o resarcimiento, condición que el Usuario acepta con el solo ingreso a la plataforma.</p>
              </div>
            </Card>
          </div>
        )}
