...
export const ModalLinksBackstage: React.FC<ComponentProps> = ({ onHide }) => {
  ...
  const handleSaveitem = (e: any, item: any) => {
    if (e.key === 'Enter') {
      if (item.id !== '' && e.target.value !== '') updateItem(e.target.value, item.id, '2')
      else if (e.target.value !== '') createItem(e.target.value, '2')
    }
  }

  const handleSave = (item: any) => {
      if (link !== '') createItem(link, '2')
  }

  const handleDeleteItem = (item: any, index: number) => {
    if (item.id !== '') {
      deleteItem(item.id, '2')
    } else {
